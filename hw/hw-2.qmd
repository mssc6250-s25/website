...
subtitle: "Due Friday, Mar 8 11:59 PM"
editor: source
...
(@) **[Special Case for Ridge and Lasso]** Suppose for a multiple linear regression problem with $n = p$, ${\bf X} = {\bf I}$ and no intercept, i.e., $y_i = \beta_i + \epsilon_i, ~~ \epsilon_i \stackrel{iid} N(0, \sigma^2)$. Show that
    ...
    (d) Show that the lasso solution of $\sum_{j=1}^p\left(y_j - \beta_j\right)^2 + \lambda \sum_{j=1}^p | \beta_j |$ is 
$$\hat{\beta}_j^l = 
  \begin{cases}
  ...
  \end{cases}$$
    ...
    (e) Describe the ridge and lasso shrinkage behavior.
